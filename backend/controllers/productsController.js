   ...
   let productsCount = totalProductsCount; 
    ...
    const products = await buildQuery().paginate(resPerPage).query;
     res.status(200).json({
         ...
         resPerPage,
         products,
        //  message:'This routs will show all products'                          
     })                              
}
...
exports.deleteReview = catchAsyncError(async (req,res,next)=>{
   ...
   //filtering the reviews which does match deleteing review id
   ...
   //number of review
    ...
    //finding the average with the filtering reviews
  let ratings =  product.reviews.reduce((acc, review)=>{
      return review.rating + acc
  },0) / product.reviews.length;
  rating = isNaN(ratings)?0:ratings;
